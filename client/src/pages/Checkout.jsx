      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4 animate-fade-in-up">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <input required type="text" placeholder="Full Name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-400 text-sm" />
            <div className="grid grid-cols-2 gap-2">
              <input required type="tel" placeholder="Phone" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-400 text-sm" />
              <input type="email" placeholder="Email (for tracking)" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-400 text-sm" />
            </div>
          </div>
          <input required type="text" placeholder="Delivery Address (full address)" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} className="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-400 text-sm" />
          {form.email && <p className="text-xs text-gray-400 flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>Order confirmation & tracking will be sent to this email</p>}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-3 animate-fade-in-up animate-delay-100">
          ...
          <div className="divide-y">
            {items.map(item => (
              <div key={item.productId} className="flex justify-between py-2 text-gray-600 text-sm">
                ...
                {orderSource === 'online_payment' && (
                  <div className="mt-4 space-y-3 animate-slide-down">
                    ...
                    <div>
                      ...
                      <input required type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => setScreenshot(e.target.files[0])} className="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-accent-50 file:text-accent-700 hover:file:bg-accent-100" />
                      <p className="text-xs text-gray-400 mt-1">Accepted: JPEG, PNG, WebP, GIF (max 20MB)</p>
                    </div>
                  </div>
                )}
              </div>
